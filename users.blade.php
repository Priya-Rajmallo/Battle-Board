...
@section('extraScript')
    ...
    <script type="text/javascript">
        ...
        function searchData() {
            ...
            dataTable.ajax.url('{{ route('users.DataTable') }}?start_date=' +
                startDate + '&end_date=' +
                endDate).load();
            // }
        }
        ...
        $(document).ready(function() {
            ...
            dataTable = $('#dataTable').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            var changeRole =
                                '<button class="btn btn-success btn-xs upgrade-btn" data-data=\'' +
                                JSON.stringify(data) + '\'>Upgrade</button>';
                ...
                // lengthMenu: [10, 25, 50, 100], // Optional: add length menu
            ...
            $('table').on('click', '.upgrade-btn', function(e) {
                e.preventDefault();
                const data = $(this).data('data');

                $('.upgrade-btn').prop('disabled', true);

                if (confirm('Are you sure you want to upgrade this normal user to promoter?')) {
                    console.log(data);
                    $.ajax({
                        url: '{{ route('promoter.upgrade') }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: 1,
