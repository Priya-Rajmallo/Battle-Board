...
@section('extraScript')
    ...
    <script type="text/javascript">
        ...
        function searchData() {
            ...
            dataTable.ajax.url('{{ route('paymentApproval.DataTable') }}?start_date=' +
                startDate + '&end_date=' +
                endDate).load();
            // }
        }
        ...
        $(document).ready(function() {
            ...
            dataTable = $('#dataTable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('paymentApproval.DataTable') }}',
                },
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            var viewLink =
                                '<a class="btn btn-primary btn-xs view-user" href="{{ route('index2') }}/images/payments/:data">View</a>';
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            if (data.status == 'pending') {
                                var btn =
                                    '<button type="button" class="btn btn-secondary btn-xs approve-btn" data-payment_id="' +
                                    data.id + '" data-user_id="' +
                                    data.user_id + '">Approval</button>';
                                return btn;
                            } else {
                                var btn =
                                    '<button type="button" class="btn btn-danger btn-xs delete-btn" data-payment_id="' +
                                    data.id + '" data-user_id="' +
                                    data.user_id + '">Delete</button>';
                                return btn;
                            }
                            return btn;
                        }
            ...
            $('table').on('click', '.approve-btn', function(e) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: "{{ route('paymentApprovalAction') }}", //backend url
                            data: {
                                _token: _token,
                                user_id: user_id,
            ...
            $('table').on('click', '.delete-btn', function(e) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.value) {
                        ...
                        event.preventDefault();
                        $.ajax({
                            url: "{{ route('paymentDelete') }}", //backend url
                            data: {
                                _token: _token,
                                user_id: user_id,
                                payment_id: payment_id,
